...
mod tests {
    ...
    fn write_time_probe() {
        let dir =
            std::env::temp_dir().join(format!("abitious-fscompress-time-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn kernel_roundtrips_incompressible_blocks() {
        let dir =
            std::env::temp_dir().join(format!("abitious-fscompress-raw-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("f.bin");
        let mut raw = Vec::new();
    ...
    fn apply_bytes_preserves_ownership_of_an_overwritten_file() {
        ...
        let dir =
            std::env::temp_dir().join(format!("abitious-fscompress-own-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("f");
