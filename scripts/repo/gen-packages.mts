...
//   node scripts/repo/gen-packages.mts --print-matrix       print the tier-1 push/PR CI matrix
//   node scripts/repo/gen-packages.mts --print-matrix-all   print the full release matrix (all targets)
//
...
function check(files: GenFile[]): number {
  ...
  if (drift.length) {
    writeStderr(
      'gen-packages --check: OUT OF SYNC with scripts/repo/targets.mts:\n',
    )
    ...
    writeStderr(
      'Fix: run `pnpm run gen` (node scripts/repo/gen-packages.mts) and commit.\n',
    )
    return 1
  }
