...
void test('workspaceVersion reads [workspace.package] version', () => {
  assert.equal(workspaceVersion(CARGO), '0.1.0')
  assert.equal(workspaceVersion('[workspace]\nmembers = []\n'), undefined)
})

void test('bumpWorkspaceCargo bumps workspace + internal pins, not external deps', () => {
  const out = bumpWorkspaceCargo(CARGO, '0.2.0')
  assert.match(out, /\[workspace\.package\]\nversion = "0\.2\.0"/)
  assert.match(
    out,
    /abitious-decmpfs = \{ path = "crates\/abitious-decmpfs", version = "0\.2\.0" \}/,
  )
  assert.match(
    out,
    /windows-sys = \{ version = "0\.59"/,
    'external dep version untouched',
  )
})

void test('bumpNpmManifest sets version + scoped optionalDependencies only', () => {
  const src = JSON.stringify(
    {
      name: '@abitious/cli',
      version: '0.1.0',
      optionalDependencies: {
        '@abitious/darwin-arm64': '0.1.0',
        'other-dep': '^1.0.0',
      },
    },
    null,
  ...
  assert.equal(
    out.optionalDependencies['other-dep'],
    '^1.0.0',
    'non-scoped dep untouched',
  )
})

void test('promoteChangelog: renames [Unreleased], keeps an existing section, else stubs', () => {
...
void test('changelogSection extracts the version body until the next heading', () => {
  const src = '# Changelog\n\n## 0.1.0\n\n- one\n- two\n\n## 0.0.9\n\n- old\n'
